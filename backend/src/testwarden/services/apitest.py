...
with the TestWarden reporter so results land in the dashboard as a normal run.
"""
import shutil
import time
...
from testwarden.db import SessionLocal
from testwarden.models import ApiKey, ApiTestJob, Project, Run
from testwarden.services.autofix import MODEL, run_agent
from testwarden.services.workspace import FixWorkspace, WorkspaceError

SPEC_CHAR_LIMIT = 80_000

SYSTEM_PROMPT = """You are TestWarden's API-testing agent. Given an OpenAPI spec, you write a \
...
def execute_apitest_job(job_id: int) -> None:
    ...
    try:
        ...
        try:
            ...
            log("Executing final suite with TestWarden reporting enabled ...")
            ...
            try:
                output = original_run_tests(
                    ...
                    extra_env={
                        ...
                        "TESTWARDEN_ENABLED": "true",
                        "TESTWARDEN_URL": settings.public_url,
                        "TESTWARDEN_API_KEY": key,
                        "TESTWARDEN_FRAMEWORK": "pytest-httpx",
                        "TESTWARDEN_BRANCH": "api-agent",
                    },
                )
            ...
            if run is not None:
                ...
                log("WARNING: no reported run found - check TESTWARDEN_URL reachability.")
