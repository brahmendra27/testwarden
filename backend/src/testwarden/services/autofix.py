...
SYSTEM_PROMPT = """You are TestWarden's autonomous fix agent. A test in this repository failed \
...
def execute_autofix_job(job_id: int) -> None:
    ...
    try:
        ...
        try:
            ...
            branch = f"testwarden/fix-{_slug(case.title)}-{job.id}"
            ...
            if verdict["outcome"] != "fixed" or not diff.strip():
                ...
                workspace.commit(f"fix: {case.title} (TestWarden auto-fix #{job.id})")
                log("Changes committed.")
                token = os.environ.get("TESTWARDEN_GITHUB_TOKEN") or os.environ.get("GITHUB_TOKEN")
                if token and workspace.github_repo():
                    pr_body = (
                        f"Automated fix for `{case.node_id}` "
                        f"(run #{run.id}, result #{result.id}).\n\n{verdict['summary']}\n\n"
                        "🤖 Generated by the TestWarden auto-fix agent — review before merging."
                    )
                    job.pr_url = workspace.push_and_open_pr(
                        branch, base_branch, f"[TestWarden] Fix {case.title}", pr_body, token
                    )
